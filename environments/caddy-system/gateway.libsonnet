...

{
  gateway: {
    ...
    serviceAccount:
      ...
      sa.metadata.withLabels($.gateway.labels) +
    ...
    clusterRole:
      ...
      cr.metadata.withLabels($.gateway.labels) +
      cr.withRules([
        rule.withApiGroups(['']) +
        rule.withResources([
          'configmaps',
    ...
    clusterRoleBinding:
      ...
      crb.metadata.withLabels($.gateway.labels) +
      crb.withSubjects([
        subject.fromServiceAccount($.gateway.serviceAccount),
      ]) +
      crb.bindRole($.gateway.clusterRole),

    local r = k.rbac.v1.role,
    role:
      r.new('caddy-gateway') +
      r.metadata.withNamespace($.namespace.metadata.name) +
      r.metadata.withLabels($.gateway.labels) +
      r.withRules([
    ...
    container::
      ...
      container.withEnv([
        ...
        env.valueFrom.resourceFieldRef.withContainerName($.gateway.container.name) +
    ...
    deployment:
      deploy.new(
        'caddy-gateway',
        replicas=1,
        containers=[$.gateway.container],
        podLabels=$.gateway.labels,
      ) +
      deploy.metadata.withNamespace($.namespace.metadata.name) +
      deploy.metadata.withLabels($.gateway.labels) +
      ...
      deploy.spec.template.spec.withTopologySpreadConstraints([
        ...
        tsc.labelSelector.withMatchLabels($.gateway.labels) +
        tsc.withMatchLabelKeys(['pod-template-hash']),
      ]) +
      deploy.spec.template.spec.withServiceAccountName($.gateway.serviceAccount.metadata.name) +
    ...
    class:
      gwc.new('caddy') +
      gwc.metadata.withLabels($.gateway.labels) +
    ...
    def:
      gateway.new($._config.caddy.gateway.name) +
      gateway.metadata.withNamespace($.namespace.metadata.name) +
      gateway.metadata.withLabels($.labels) +
      gateway.spec.withGatewayClassName($.gateway.class.metadata.name) +
